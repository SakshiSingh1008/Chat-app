...
const{fullName,email,password}=req.body;
  ...
  if(!fullName||!email||!password){
    res.status(400).send('Please fill all requires fields');
  }else{
    ...
    if(isAlreadyExist) {
      res.status(400).send('User already exists');
       ...
       return res.status(200).send('User registered succuessfully');
    }
  }
...
app.post('/api/message',async(req,res)=>{
  try{
    ...
    if(!senderId||!message) return res.status(400).send('Please fill all require fields')
